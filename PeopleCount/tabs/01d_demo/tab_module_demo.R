...
tab_demo_ui <- function(id) {
  ...
  fluidPage(
    ...
    fluidRow (
      tags$em("Remember: Input videos will give best results only when they are recorded at perticular angles, with height 28 and width 26 , frame size 640*480.", style = 'color: orange;font-size:110%')
    ),
      br(),
    fluidRow(
      column(4,
    ...
    fluidRow(column (10, offset = 1,
                     hidden(
                       div (id = "detect_control_ui",
                            withBusyIndicatorUI(
                              actionButton("detect", "Perform Counting")
                            ))
                     ))),
    br(),
    fluidRow(column (10, offset = 1,
...
tab_demo_server <- function(input, output, session) {
  ...
  observeEvent(input$detect, {
    withBusyIndicatorServer("detect", {
      ...
      detect_python <<- import_from_path("people_count_AWS")
      detect_python_new <<- import_from_path("ConvertVideo")

      
      detect_python$performdetection(input_path,
                                   output_path)
