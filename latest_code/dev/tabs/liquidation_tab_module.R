...
liquidation_tab_server <- function(input, output, session, createvals) {
  ...
  observeEvent(input$optimise,
               {
                 ...
                 max_pre <- max(max_price/base_price,1.05)
  ...
  output$optim_desc <- DT::renderDataTable({
    ...
    if(is.data.frame(optim_df)){
      ...
      print("here we are at optim_df") 
      print(optim_df)
      write.csv(optim_df,'optim_df.csv')
      ...
      optim_df <- data.table::rbindlist(lkpis_res, idcol="kpi") %>%
        ...
        data.table::dcast(actualvsoptimal ~ kpi) %>% 
        tibble::column_to_rownames('actualvsoptimal') %>%
        select(margin, revenue
               #,units_left ,units_left_cost
               ,average_price ) %>%
