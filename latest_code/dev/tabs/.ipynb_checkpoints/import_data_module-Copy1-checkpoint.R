...
and shop not in ('0048','0245','1002','1003','1004' )
and  PRODUCT not like 'DIESEL%'
and net_price > 0
and APPLIED_MKDOWN > 0 
        and TransactionID not in (SELECT distinct TransactionID FROM `driven-data-proto.teradata.ReceiptHeaders` 
            where length(trim(FleetAcctNum)) > 0)
        and shop not in ('0048','0228','0245','1002','1003','1004','0323','0352','0356','0370','0390') ## added 5 havoline stores 11/14
...
AVg(LICOST)            AS  COST,
avg(PRICE) as PRICE
FROM `driven-teradata.t5analytics.Final_View_Test` 
where  PRODDUCTGROUP in ('BIG4', 'OIL CHANGE')
 and  PRODUCT not like 'DIESEL%'
group by 1,2,3,4,5
"
