  ...
  fluidPage(
    fluidRow(
      box(width=12, align='left', title = 'Forecast Evaluation',
          column(
            ...
            column(style = "margin-top: 25px;",actionButton(ns('forecast'), 'Forecast'), width=2),
            width=12)
      )),
       fluidRow(
    ...
    fluidRow(
      box(width=12, align='left', title = 'Forecast Components',
          column(
            selectInput(ns('component'), 'Select Component', choices=c('Trend', 'Day of Week', 'Day of year', 'Extra regressors'
...
forecast_tab_server <- function(input, output, session) {
  ...
  print("model Name from model selection ........................")
  #print(model_name)
  ...
  observeEvent(input$forecast, 
               {
                 withProgress({
                  ...
                  # print("printing model_name...")
                   ...
                   #print(reactive_list$workflow@model_factory@models[model_name])
                     
                   filter_factors <- unlist(strsplit(model_name, "__"))
                   #print("we are here .....")
                    #print(filter_factors ) 
                  ...
                  print(reactive_list$workflow@data@training_set) 
                   ...
                   print("we are recheched till name ")
                   print(reactive_list$plot_components[[model_name]]) 
                     
                     
                   #names(reactive_list$plot_components[[model_name]]) <- c('Trend', 'Day of Week', 'Day of year', 'Extra regressors')
                   names(reactive_list$plot_components[[model_name]]) <- c('Trend', 'Day of Week', 'Day of year')
                   
                   reactive_list$forecasts[[model_name]] <- forecasts
                   },
                   min = 0,
  ...
  #print("printing value of model name after product Name")
    
    model_names <- reactive_list$trained_models
      print(length(model_names)) 
    if(length(model_names) > 0){
      prod_choices <- unique(sapply(strsplit(model_names, "__"), function(v) v[1]))                                       
      print(model_names)
      #CONVENTIONAL__0301
    #  print(strsplit(model_names, "__"))
     # print("Printing sapply Values ....")
      #print(sapply(strsplit(model_names, "__"), function(v) v[1]))
      #print("Printing Unique Values ....")
      #print(unique(sapply(strsplit(model_names, "__"), function(v) v[1])))
      #print("prod choice .......")
      #print(unique(sapply(strsplit(model_names, "__"), function(v) v[1])))
      ...
      #prod_choices <- unique((reactive_list$products$PRODUCT))
      #print("printing PRODUCT ...........")
      #print(PRODUCT)
      #print("printing prod_choices ...........")
      #print(prod_choices)
 ...
 product_num <- reactive({
  ...
  output$componentplot <- renderPlotly({
    req(input$store, product_num(), reactive_list$plot_components[[model_selection()]], input$component)
    ...
    p <- reactive_list$plot_components[[model_selection()]][[input$component]]
    p
  })
  
  output$performanceplot <- renderPlotly({
    req(input$store, product_num(), reactive_list$plot_components[[model_selection()]], input$metric)
