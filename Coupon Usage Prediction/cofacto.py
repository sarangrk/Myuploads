...
def _solve_weighted_factor(lo, hi, beta, X, BTBpR, c0, c1, f, lam_theta):
    print('inside func _solve_weighted_factor')
    ...
    for ib, u in enumerate(range(lo, hi)):
        print('inside for loop: ib,u = ' + str(ib) + ',' + str(u))
        ...
        print('BTBpR:',BTBpR)
        print('B_u.T:',B_u.T)
        print('(c1 - c0) * B_u:',(c1 - c0) * B_u)
        print('B:',B)
        theta_batch[ib] = LA.solve(B, a)
        print('theta_batch[ib]:',theta_batch[ib])
    return theta_batch
