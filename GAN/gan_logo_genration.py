...
def load_data(pixels=128, verbose=False):
    print("Loading data")
    X_train = []
    paths = glob.glob(os.path.normpath(os.getcwd() + '/Alphabet_Real/*.png'))
    for path in paths:
    ...
    for epoch in range(epochs):
        ...
        for index in range(int(X_train.shape[0]/BATCH_SIZE)):
            ...
            if index % 20 == 0 and epoch % 10 == 0:
                ...
                destpath = os.path.normpath(os.getcwd()+ "/Alphabet_Fake/"+str(epoch)+"_"+str(index)+".png")
...
def generate(BATCH_SIZE):
    ...
    for image in generated_images:
        image = image[0]
        image = image*127.5+127.5
        Image.fromarray(image.astype(np.uint8)).save("dirty_2.png")
        Image.fromarray(image.astype(np.uint8)).show()
        clean(image)
        image = Image.fromarray(image.astype(np.uint8))
        image.show()
        image.save("clean_2.png")
